...
# NOTE: Using "Insurance.csv" which must be present in the same directory as this file.
@st.cache_data
def load_and_preprocess_data(data_path="Insurance.csv"):
    """Loads, cleans, imputes, and encodes the dataset."""
    try:
        # Check if the file path is accessible in the Streamlit environment
        # If running locally, this will load the file; if running on Cloud, it must be uploaded.
        df = pd.read_csv(data_path)
    except FileNotFoundError:
        # In a real Streamlit Cloud environment, if the file is not uploaded/provided, this will fail.
        st.error(f"Error: The dataset '{data_path}' was not found. Please ensure it is uploaded.")
...
def train_models(X, y, le):
    ...
    if X.empty or y.empty:
        return {}, pd.DataFrame(), pd.DataFrame(), pd.Series(), pd.Series(), [], le
...
def generate_performance_metrics(trained_models, X_train, X_test, y_train, y_test, le):
    ...
    if X_train.empty or X_test.empty:
        st.error("Model data is empty. Cannot generate metrics.")
        return
    ...
    for name, model in trained_models.items():
        ...
        try:
            cv_accuracy = np.mean(cross_val_score(model, X_train, y_train, cv=cv, scoring='accuracy', n_jobs=-1))
             ...
             cv_accuracy = np.nan # Handle cross_val_score errors gracefully
        ...
        precision = precision_score(y_test, y_test_pred, zero_division=0) # Use zero_division=0 for robustness
...
def make_predictions(df_to_predict, trained_models, feature_columns, le):
    ...
    df_insights, _, _, le_ref = load_and_preprocess_data()
    
    # 2. Imputation with mode/median from training data
    for col in ['PI_OCCUPATION', 'REASON_FOR_CLAIM']:
        # Fallback to a safe string if mode cannot be determined
        mode_val = df_insights[col].mode().iloc[0] if not df_insights[col].mode().empty else 'UNKNOWN'
    ...
    if best_model is None:
        return pd.DataFrame() # Return empty if models failed to train
...
# NOTE: The file "Insurance.csv" is assumed to be present in the same directory.
df_insights, X_encoded, y, le = load_and_preprocess_data(data_path="Insurance.csv")
...
if not df_insights.empty:
    ...
    st.title("🛡️ Insurance Policy Status Prediction Dashboard")
    st.markdown("This application provides data insights and machine learning model evaluation for Policy Status analysis.")

    # Create tabs
    tab_insights, tab_model, tab_predict = st.tabs(["📊 Policy Insights", "🔬 Model Performance", "⬆️ New Prediction"])
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Predict Policy Status (using Random Forest)"):
                    ...
                    predicted_df = make_predictions(new_df, trained_models, feature_cols, le)
                    ...
                    # Download link
                    st.markdown(
                        get_table_download_link(predicted_df, "predicted_policy_status.csv", "⬇️ Download Predicted Data"), 
